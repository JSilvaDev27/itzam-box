...
impl ContainerEngine for DockerLinuxEngine {
    // ─── Diagnostics ───
    async fn check_engine_status(&self) -> Result<bool, String> { Ok(Self::is_available()) }

    async fn get_engine_version(&self) -> Result<String, String> {
        if !Self::is_available() { return Err("Docker daemon not available".into()); }
        ...
        for line in output.lines() {
            ...
            if let Ok(raw) = serde_json::from_str::<serde_json::Value>(line) {
                containers.push(ContainerInfo {
                    ...
                    compose_project: raw.get("Labels")
    ...
    async fn rename_container(&self, id: &str, new_name: &str) -> Result<(), String> {
        Self::run_docker(&["rename", id, new_name])?; Ok(())
    }
    async fn remove_container(&self, id: &str, force: bool, remove_volumes: bool) -> Result<(), String> {
        let mut args = vec!["rm"];
        if force { args.push("-f"); }
        if remove_volumes { args.push("-v"); }
        args.push(id);
        Self::run_docker(&args)?; Ok(())
    }
    ...
    async fn get_container_stats(&self, id: &str) -> Result<ContainerStats, String> {
        ...
        Ok(ContainerStats {
            ...
            cpu_percentage: raw_get(&raw, "CPUPerc").trim_end_matches('%').parse().unwrap_or(0.0),
            memory_usage_bytes: parse_mem(&raw_get(&raw, "MemUsage")),
            memory_limit_bytes: 0,
            memory_percentage: raw_get(&raw, "MemPerc").trim_end_matches('%').parse().unwrap_or(0.0),
    ...
    async fn create_and_run_container(&self, image: &str, name: Option<&str>,
        ports: Vec<PortMapping>, volumes: Vec<String>, env_vars: Vec<String>,
        network: Option<&str>, restart_policy: Option<&str>,
        command: Option<Vec<String>>, detach: bool) -> Result<String, String>
    {
        let mut args: Vec<String> = vec!["run".to_string()];
        if detach { args.push("-d".into()); }
        if let Some(n) = name { args.push("--name".into()); args.push(n.to_string()); }
        for p in &ports {
            args.push("-p".into());
            args.push(format!("{}:{}:{}/{}", p.host_ip, p.host_port, p.container_port, p.protocol));
        }
        for v in &volumes { args.push("-v".into()); args.push(v.clone()); }
        for e in &env_vars { args.push("-e".into()); args.push(e.clone()); }
        if let Some(net) = network { args.push("--network".into()); args.push(net.to_string()); }
        if let Some(rp) = restart_policy { args.push("--restart".into()); args.push(rp.to_string()); }
        args.push(image.to_string());
        if let Some(cmd) = &command { for c in cmd { args.push(c.clone()); } }
    ...
    async fn download_file_from_container(&self, cid: &str, remote: &str, local: PathBuf) -> Result<(), String> {
        Self::run_docker(&["cp", &format!("{}:{}", cid, remote), local.to_str().unwrap_or("/tmp/out")])?; Ok(())
    }
    async fn upload_file_to_container(&self, cid: &str, local: PathBuf, remote: &str) -> Result<(), String> {
        Self::run_docker(&["cp", local.to_str().unwrap_or("/tmp/in"), &format!("{}:{}", cid, remote)])?; Ok(())
    }
    async fn read_file_preview(&self, cid: &str, remote: &str, max: usize) -> Result<String, String> {
    ...
    async fn inspect_image(&self, id: &str) -> Result<serde_json::Value, String> {
        ...
        let vals: Vec<serde_json::Value> = serde_json::from_str(&output).map_err(|e| format!("Parse: {}", e))?;
    ...
    async fn pull_image(&self, image_name: &str) -> Result<(), String> {
        Self::run_docker(&["pull", image_name])?; Ok(())
    }
    ...
    async fn tag_image(&self, id: &str, repo: &str, tag: &str) -> Result<(), String> {
        Self::run_docker(&["tag", id, &format!("{}:{}", repo, tag)])?; Ok(())
    }
    ...
    async fn create_volume(&self, name: &str, driver: Option<&str>) -> Result<(), String> {
        ...
        if let Some(d) = driver { args.push("-d"); args.push(d); }
        args.push(name);
        Self::run_docker(&args)?; Ok(())
    }
        ...
        for line in output.lines() {
            if let Ok(raw) = serde_json::from_str::<serde_json::Value>(line.trim()) {
                networks.push(NetworkInfo {
                    ...
                    subnet: None, gateway: None, internal: false,
                    containers: vec![],
                });
            }
        }
    ...
    async fn create_network(&self, name: &str, driver: &str, subnet: Option<&str>, gateway: Option<&str>) -> Result<(), String> {
        ...
        if let Some(s) = subnet { args.push("--subnet"); args.push(s); }
        if let Some(g) = gateway { args.push("--gateway"); args.push(g); }
        args.push(name);
        Self::run_docker(&args)?; Ok(())
    }
    async fn remove_network(&self, id: &str) -> Result<(), String> {
        Self::run_docker(&["network", "rm", id])?; Ok(())
    }
    ...
    async fn get_disk_usage(&self) -> Result<DiskUsageSummary, String> {
        ...
        let mut summary = DiskUsageSummary {
            containers_count: 0, containers_size_bytes: 0,
            images_count: 0, images_size_bytes: 0,
            volumes_count: 0, volumes_size_bytes: 0,
            build_cache_size_bytes: 0, total_reclaimable_bytes: 0,
        };
        for line in output.lines() {
            if let Ok(raw) = serde_json::from_str::<serde_json::Value>(line.trim()) {
                ...
                match stype.as_str() {
                    "Containers" => { summary.containers_count = count; summary.containers_size_bytes = size; }
                    "Images" => { summary.images_count = count; summary.images_size_bytes = size; }
                    "Local Volumes" => { summary.volumes_count = count; summary.volumes_size_bytes = size; }
                    "Build Cache" => { summary.build_cache_size_bytes = size; }
                    _ => {}
                }
    ...
    async fn prune_containers(&self) -> Result<u64, String> {
        let output = Self::run_docker(&["container", "prune", "-f"])?; Ok(parse_size(&output))
    }
    async fn prune_images(&self, dangling_only: bool) -> Result<u64, String> {
        let mut args = vec!["image", "prune", "-f"];
        if !dangling_only { args.push("-a"); }
        let output = Self::run_docker(&args)?; Ok(parse_size(&output))
    }
    async fn prune_volumes(&self) -> Result<u64, String> {
        let output = Self::run_docker(&["volume", "prune", "-f"])?; Ok(parse_size(&output))
    }
    async fn prune_networks(&self) -> Result<u64, String> {
        let output = Self::run_docker(&["network", "prune", "-f"])?; Ok(parse_size(&output))
    }
    async fn prune_build_cache(&self) -> Result<u64, String> {
        let output = Self::run_docker(&["builder", "prune", "-f"])?; Ok(parse_size(&output))
    }
    async fn system_prune_all(&self) -> Result<DiskUsageSummary, String> {
        Self::run_docker(&["system", "prune", "-f"])?;
        DiskUsageSummary { containers_count:0, containers_size_bytes:0, images_count:0, images_size_bytes:0, volumes_count:0, volumes_size_bytes:0, build_cache_size_bytes:0, total_reclaimable_bytes:0 };
...
fn raw_get(val: &serde_json::Value, key: &str) -> String {
    val.get(key).map(|v| v.as_str().unwrap_or("").to_string()).unwrap_or_default()
}

fn parse_size(s: &str) -> u64 {
    let s = s.trim().to_uppercase();
    if s.is_empty() { return 0; }
    let num: f64 = s.chars().take_while(|c| c.is_digit(10) || *c == '.').collect::<String>().parse().unwrap_or(0.0);
    if s.ends_with("GB") || s.ends_with("G") { return (num * 1_000_000_000.0) as u64; }
    if s.ends_with("MB") || s.ends_with("M") { return (num * 1_000_000.0) as u64; }
    if s.ends_with("KB") || s.ends_with("K") { return (num * 1_000.0) as u64; }
    if s.ends_with("B") { return num as u64; }
    s.parse().unwrap_or(0)
}

fn parse_mem(s: &str) -> u64 {
    // Format: "45MiB / 512MiB" or "45.2MiB / 512MiB"
    s.split('/').next().unwrap_or("0").trim().replace("MiB","").replace("GiB","000")
        .parse::<f64>().map(|v| (v * 1_048_576.0) as u64).unwrap_or(0)
}
