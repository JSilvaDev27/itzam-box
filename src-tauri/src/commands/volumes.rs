...
#[tauri::command]
pub async fn create_volume(state: State<'_, AppState>, name: String, driver: Option<String>) -> Result<(), String> {
    state.engine.create_volume(&name, driver.as_deref()).await
}

#[tauri::command]
pub async fn remove_volume(state: State<'_, AppState>, name: String, force: bool) -> Result<(), String> {
