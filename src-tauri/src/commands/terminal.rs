...
pub async fn spawn_host_terminal(
    ...
    pty_state.sessions.lock().map_err(|e| e.to_string())?.insert(id.clone(), session);
...
pub async fn pty_write(
    ...
    s.writer.write_all(data.as_bytes()).map_err(|e| format!("Write error: {}", e))?;
    s.writer.flush().map_err(|e| format!("Flush error: {}", e))?;
    Ok(())
}
