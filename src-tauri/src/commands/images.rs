...
#[tauri::command]
pub async fn remove_image(state: State<'_, AppState>, id: String, force: bool) -> Result<(), String> {
    state.engine.remove_image(&id, force).await
}

#[tauri::command]
pub async fn tag_image(state: State<'_, AppState>, id: String, repository: String, tag: String) -> Result<(), String> {
    state.engine.tag_image(&id, &repository, &tag).await
}

#[tauri::command]
pub async fn inspect_image(state: State<'_, AppState>, id: String) -> Result<serde_json::Value, String> {
