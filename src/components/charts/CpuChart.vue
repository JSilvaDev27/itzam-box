<!-- ItzamBox — CPU Time-Series Chart Component
...
const lineColor = computed(() => {
  if (props.data.length === 0) return getCssVar('--accent-green') || '#10b981'
  const latest = props.data[props.data.length - 1].y
  return latest > 80 ? (getCssVar('--accent-red') || '#ef4444') : (getCssVar('--accent-green') || '#10b981')
})
...
function createOrUpdateChart() {
  ...
  if (!chartInstance) {
    ...
    chartInstance.data.datasets[0].borderColor = lineColor.value
    chartInstance.data.datasets[0].backgroundColor = `${lineColor.value}20`
...
<template>
  <div class="chart-card">
    <div class="chart-header">
      <span class="chart-label">CPU Usage</span>
      <span class="chart-meta">{{ cores }} cores</span>
    </div>
